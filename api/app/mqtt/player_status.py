...
def _on_status_message(client: mqtt_client.Client, userdata: Any, msg: mqtt_client.MQTTMessage) -> None:
    ...
    try:
        ...
        try:
            player = db.query(models.Player).filter(models.Player.player_key == player_key).first()
            
            if not player:
                logger.warning(f"Received status update for unknown player_key: {player_key}")
                return
