...
def _on_view_message(client: mqtt_client.Client, userdata: Any, msg: mqtt_client.MQTTMessage) -> None:
    ...
    try:
        ...
        if len(parts) != 4 or parts[0] != "makapix" or parts[1] != "player" or parts[3] != "view":
        ...
        try:
            ...
            from ..services.rate_limit import check_player_view_rate_limit, check_view_duplicate
            ...
            if view_event.intent == "artwork":
                ...
                logger.warning(f"Unexpected intent value: {view_event.intent}, defaulting to LISTING")
                view_type = ViewType.LISTING
            ...
            if view_event.timestamp != UNSYNC_TIMESTAMP:
                ...
                logger.debug(f"Player {player_key} has unsynced time, storing NULL for local_datetime")
        ...
        try:
            ...
            def on_connect(client, userdata, flags, reason_code, properties):
                if reason_code == 0:
                    ...
                    logger.error(f"View event subscriber connection failed: {reason_code}")
            ...
            _view_client = client
            logger.info(f"Player view event subscriber started (broker: {broker_host}:{broker_port})")
