...
def create_verification_token(db: Session, user_id: UUID, email: str) -> str:
    ...
    recent_tokens = db.query(models.EmailVerificationToken).filter(
    ...
    if recent_tokens >= MAX_TOKENS_PER_HOUR:
        raise ValueError(f"Rate limit exceeded. Maximum {MAX_TOKENS_PER_HOUR} verification emails per hour.")
...
def mark_email_verified(db: Session, token: str) -> models.User | None:
    ...
    if not user:
        logger.error(f"User {verification_token.user_id} not found for verification token")
        return None
...
def get_pending_verifications(db: Session, user_id: UUID) -> list[models.EmailVerificationToken]:
    """
...
def invalidate_pending_verifications(db: Session, user_id: UUID) -> int:
    ...
    """
    now = datetime.now(timezone.utc)
    count = db.query(models.EmailVerificationToken).filter(
        models.EmailVerificationToken.user_id == user_id,
        models.EmailVerificationToken.used_at.is_(None),
        models.EmailVerificationToken.expires_at > now,
    ).update({"used_at": now})
