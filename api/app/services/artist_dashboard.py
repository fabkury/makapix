...
class ArtistDashboardService:
    ...
    def get_artist_stats(self, user_key: UUID) -> ArtistStats | None:
        ...
        # Verify user exists
        user = self.db.query(models.User).filter(models.User.user_key == user_key).first()
        if not user:
            return None
    ...
    def get_posts_stats_list(self, user_key: UUID, limit: int = 20, offset: int = 0) -> list[PostStatsListItem]:
        """
        ...
        for post in posts:
            ...
            if stats:
                result.append(PostStatsListItem(
                    post_id=post.id,
...
def get_posts_stats_list(db: Session, user_key: UUID, limit: int = 20, offset: int = 0) -> list[PostStatsListItem]:
    """
