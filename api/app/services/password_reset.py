...
def create_reset_token(db: Session, user_id: UUID) -> str:
    ...
    # Check rate limit
    recent_tokens_count = db.query(models.PasswordResetToken).filter(
        models.PasswordResetToken.user_id == user_id,
        models.PasswordResetToken.created_at >= datetime.now(timezone.utc) - timedelta(hours=RESET_RATE_LIMIT_HOURS)
    ).count()
    
    if recent_tokens_count >= RESET_RATE_LIMIT_COUNT:
        raise ValueError(f"Rate limit exceeded. Please wait before requesting another password reset.")
    
    # Generate secure token
    plain_token = secrets.token_urlsafe(32)
    token_hash = _hash_token(plain_token)
    expires_at = datetime.now(timezone.utc) + timedelta(hours=PASSWORD_RESET_TOKEN_EXPIRATION_HOURS)
