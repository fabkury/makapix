...
class BlogPostStatsService:
    ...
    def get_blog_post_stats(self, blog_post_id: int) -> BlogPostStats | None:
        ...
        blog_post = self.db.query(models.BlogPost).filter(models.BlogPost.id == blog_post_id).first()
        if not blog_post:
            return None
    ...
    def _compute_stats(self, blog_post_id: int) -> BlogPostStats:
        ...
        recent_views = self.db.query(models.BlogPostViewEvent).filter(
        ...
        daily_stats = self.db.query(models.BlogPostStatsDaily).filter(
        ...
        if recent_views:
            ...
            oldest_daily = self.db.query(models.BlogPostStatsDaily).filter(
                models.BlogPostStatsDaily.blog_post_id == blog_post_id,
                models.BlogPostStatsDaily.total_views > 0
            ).order_by(models.BlogPostStatsDaily.date.asc()).first()
        ...
        reactions = self.db.query(models.BlogPostReaction).filter(
            models.BlogPostReaction.blog_post_id == blog_post_id
        ).all()
        ...
        comments = self.db.query(models.BlogPostComment).filter(
