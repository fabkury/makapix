...
def list_user_repositories(
    ...
    try:
        ...
        # List repositories
        try:
            logger.info(f"Attempting to list repositories for installation {installation.installation_id}")
            repos = list_repositories(token, installation.installation_id)
            logger.info(f"Successfully fetched {len(repos)} repositories")
            if len(repos) == 0:
                logger.warning(f"No repositories returned for installation {installation.installation_id}. This might indicate:")
                logger.warning("1. The installation token doesn't have access to repositories")
            ...
            logger.warning(f"Installation ID: {installation.installation_id}, Account: {installation.account_login}")
        ...
        for repo in repos:
            try:
                ...
                logger.warning(f"Repository missing required field: {e}, skipping repo: {repo}")
                continue
...
def create_user_repository(
    ...
    try:
        ...
        try:
            ...
            logger.error(f"HTTP error creating repository '{repo_name}': {e}", exc_info=True)
            ...
            if "Resource not accessible by integration" in error_detail or "403" in str(e):
                error_detail = (
            ...
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN if "Resource not accessible" in error_detail else status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail=f"Failed to create repository: {error_detail}"
            )
        except Exception as e:
            logger.error(f"Failed to create repository '{repo_name}': {e}", exc_info=True)
            ...
            # Check if it's a permissions error
            if "Resource not accessible by integration" in error_detail or "403" in error_detail:
                error_detail = (
                    "GitHub App installation token does not have permission to create repositories. "
                    "Please create the repository manually on GitHub, then refresh the list to select it. "
                    "Alternatively, you can update your GitHub App permissions to include 'Repository creation' permission."
...
async def relay_upload(
    ...
    logger.info(f"Creating relay job for repository: {repo_name} (user: {current_user.id})")
