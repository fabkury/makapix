...
def bind_github_app(
    ...
    logger.info(
        f"Verifying installation {payload.installation_id} belongs to configured GitHub App"
    )
    ...
    for identity in identities:
        if identity.provider == "github":
            github_username = (
                identity.provider_metadata.get("username")
    ...
    existing = (
        db.query(models.GitHubInstallation)
        .filter(models.GitHubInstallation.installation_id == payload.installation_id)
        .first()
    )
    ...
    logger.info(
        f"Creating new installation binding for user {current_user.id}, installation {payload.installation_id}"
    )
