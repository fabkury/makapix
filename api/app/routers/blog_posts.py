...
def list_blog_posts(
    ...
    sort: str = Query(
        "created_at", regex="^(created_at|updated_at|reactions|comments)$"
    ),
    ...
    is_viewing_own_posts = (
        owner_id
        and isinstance(current_user, models.User)
    ...
    if sort == "reactions":
        ...
        query = query.outerjoin(
            reaction_counts, models.BlogPost.id == reaction_counts.c.blog_post_id
        )
        ...
        query = query.outerjoin(
            comment_counts, models.BlogPost.id == comment_counts.c.blog_post_id
        )
        ...
        query = apply_cursor_filter(
            query, models.BlogPost, cursor, "updated_at", sort_desc=sort_desc
        )
        query = query.order_by(
            models.BlogPost.updated_at.desc()
            if sort_desc
            else models.BlogPost.updated_at.asc()
        )
    else:  # created_at
        query = apply_cursor_filter(
            query, models.BlogPost, cursor, "created_at", sort_desc=sort_desc
        )
        query = query.order_by(
            models.BlogPost.created_at.desc()
            if sort_desc
            else models.BlogPost.created_at.asc()
        )
...
def get_blog_post_by_sqid(
    ...
    if not blog_post.visible:
        if (
            not current_user
            or not isinstance(current_user, models.User)
            or not check_ownership(blog_post.owner_id, current_user)
        ):
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Blog post not found"
            )
    ...
    if blog_post.hidden_by_mod:
        if (
            not current_user
            or not isinstance(current_user, models.User)
            or not ("moderator" in current_user.roles or "owner" in current_user.roles)
        ):
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Blog post not found"
            )
    ...
    try:
        ...
        if blog_post and blog_post.public_sqid:
            ...
            return RedirectResponse(
                url=f"/api/blog-post/b/{blog_post.public_sqid}",
...
async def upload_blog_image(
    ...
    try:
        ...
        return schemas.BlogPostImageUploadResponse(
...
def create_blog_comment(
    ...
    comment_count = (
        db.query(func.count(models.BlogPostComment.id))
        .filter(models.BlogPostComment.blog_post_id == id)
        .scalar()
    )
    ...
    if payload.parent_id:
        parent = (
            db.query(models.BlogPostComment)
            .filter(models.BlogPostComment.id == payload.parent_id)
            .first()
        )
...
def update_blog_comment(
    ...
    comment = (
        db.query(models.BlogPostComment)
        .filter(models.BlogPostComment.id == commentId)
        .first()
    )
    if not comment:
        raise HTTPException(
...
def get_blog_reactions(
    ...
    _check_blog_posts_feature_lock()  # FEATURE POSTPONED - Remove this line to reactivate
    reactions = (
        db.query(models.BlogPostReaction)
        .filter(models.BlogPostReaction.blog_post_id == id)
        .all()
    )
    ...
    for reaction in reactions:
        ...
        if (
            ...
            and reaction.user_ip == current_user.ip
        ):
            if emoji not in mine:
                mine.append(emoji)
...
def add_blog_reaction(
    ...
    if isinstance(current_user, models.User):
        ...
        reaction_count = (
            db.query(func.count(models.BlogPostReaction.id))
            .filter(
                models.BlogPostReaction.blog_post_id == id,
                models.BlogPostReaction.user_id == current_user.id,
            )
...
async def get_blog_post_statistics(
    ...
    **Response includes:**
    ...
    return schemas.BlogPostStatsResponse(
        ...
        # Timestamps
        first_view_at=(
            datetime.fromisoformat(stats.first_view_at) if stats.first_view_at else None
        ),
        last_view_at=(
            datetime.fromisoformat(stats.last_view_at) if stats.last_view_at else None
        ),
