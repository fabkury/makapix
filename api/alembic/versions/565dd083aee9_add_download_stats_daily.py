...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column(
            "downloads_human", sa.Integer(), nullable=False, server_default=sa.text("0")
        ),
        sa.Column(
            "downloads_bot", sa.Integer(), nullable=False, server_default=sa.text("0")
        ),
        sa.Column(
            "updated_at",
        ...
        sa.UniqueConstraint(
            "post_id", "date", name="uq_download_stats_daily_post_date"
        ),
    )
    op.create_index(
        "ix_download_stats_daily_post_id", "download_stats_daily", ["post_id"]
    )
    op.create_index("ix_download_stats_daily_date", "download_stats_daily", ["date"])
    op.create_index(
        "ix_download_stats_daily_post_date", "download_stats_daily", ["post_id", "date"]
    )


def downgrade() -> None:
    op.drop_index(
        "ix_download_stats_daily_post_date", table_name="download_stats_daily"
    )
