...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "view_source", sa.String(20), nullable=False
        ),  # web, api, widget, player
        sa.Column(
            "view_type", sa.String(20), nullable=False
        ),  # intentional, listing, search, widget
        sa.Column(
            "user_agent_hash", sa.String(64), nullable=True
    ...
    op.create_index(
        "ix_view_events_post_created", "view_events", ["post_id", "created_at"]
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "views_by_type", postgresql.JSONB(), nullable=False, server_default="{}"
        ),
        sa.Column(
            "created_at",
            sa.DateTime(timezone=True),
    ...
    op.create_index(
        "ix_post_stats_cache_expires_at", "post_stats_cache", ["expires_at"]
    )
    ...
    op.execute(
        f'GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE view_events TO "{api_worker_user}"'
    )
    op.execute(
        f'GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE post_stats_daily TO "{api_worker_user}"'
    )
    op.execute(
        f'GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE post_stats_cache TO "{api_worker_user}"'
    )
    op.execute(
        f'GRANT USAGE, SELECT ON SEQUENCE post_stats_daily_id_seq TO "{api_worker_user}"'
    )
    op.execute(
        f'GRANT USAGE, SELECT ON SEQUENCE post_stats_cache_id_seq TO "{api_worker_user}"'
    )
