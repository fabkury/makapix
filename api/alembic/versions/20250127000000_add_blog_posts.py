...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        ),  # Markdown content, up to 10,000 chars
        sa.Column(
            "image_urls",
    ...
    op.execute(
        f'GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE blog_posts TO "{api_worker_user}"'
    )
    op.execute(
        f'GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE blog_post_comments TO "{api_worker_user}"'
    )
    op.execute(
        f'GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE blog_post_reactions TO "{api_worker_user}"'
    )
    op.execute(
        f'GRANT USAGE, SELECT ON SEQUENCE blog_post_reactions_id_seq TO "{api_worker_user}"'
    )
...
def downgrade() -> None:
    ...
    op.drop_index(
        "ix_blog_post_comments_hidden_by_mod", table_name="blog_post_comments"
    )
