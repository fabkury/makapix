...
def upgrade() -> None:
    ...
    op.add_column(
        "posts",
        sa.Column("new_id", sa.Integer(), nullable=True, server_default=text("nextval('posts_new_id_seq')")),
    )
    ...
    op.execute("SELECT setval('posts_new_id_seq', COALESCE((SELECT MAX(new_id) FROM posts), 0) + 1, false)")
    ...
    op.alter_column("reactions", "post_id_new", new_column_name="post_id", nullable=False)
    ...
    op.alter_column("admin_notes", "post_id_new", new_column_name="post_id", nullable=False)
    ...
    op.alter_column("view_events", "post_id_new", new_column_name="post_id", nullable=False)
    
    # Post stats daily table - update data, drop old FK
    op.add_column("post_stats_daily", sa.Column("post_id_new", sa.Integer(), nullable=True))
    op.execute("""
    ...
    op.drop_constraint("post_stats_daily_post_id_fkey", "post_stats_daily", type_="foreignkey")
    op.drop_column("post_stats_daily", "post_id")
    op.alter_column("post_stats_daily", "post_id_new", new_column_name="post_id", nullable=False)
    
    # Post stats cache table - update data, drop old FK
    op.add_column("post_stats_cache", sa.Column("post_id_new", sa.Integer(), nullable=True))
    op.execute("""
    ...
    op.drop_constraint("post_stats_cache_post_id_fkey", "post_stats_cache", type_="foreignkey")
    op.drop_column("post_stats_cache", "post_id")
    op.alter_column("post_stats_cache", "post_id_new", new_column_name="post_id", nullable=False)
    ...
    op.add_column("playlists", sa.Column("post_ids_new", postgresql.ARRAY(sa.Integer()), nullable=True))
    op.execute("""
        UPDATE playlists
    ...
    op.execute("ALTER TABLE posts ALTER COLUMN id SET DEFAULT nextval('posts_new_id_seq')")
    ...
    op.create_foreign_key("reactions_post_id_fkey", "reactions", "posts", ["post_id"], ["id"], ondelete="CASCADE")
    ...
    op.create_foreign_key("admin_notes_post_id_fkey", "admin_notes", "posts", ["post_id"], ["id"], ondelete="CASCADE")
    op.create_index("ix_admin_notes_post_id", "admin_notes", ["post_id"])
    
    # View events table - create FK and indexes
    op.create_foreign_key("view_events_post_id_fkey", "view_events", "posts", ["post_id"], ["id"], ondelete="CASCADE")
    ...
    op.create_foreign_key("post_stats_daily_post_id_fkey", "post_stats_daily", "posts", ["post_id"], ["id"], ondelete="CASCADE")
    ...
    op.alter_column("reports", "target_id_str", new_column_name="target_id", nullable=False)
    ...
    # Add new string column
    op.add_column("audit_logs", sa.Column("target_id_str", sa.String(50), nullable=True))
    
    # Copy data, converting UUID to string
    op.execute("UPDATE audit_logs SET target_id_str = target_id::text WHERE target_id IS NOT NULL")
    
    # Drop old column and rename new one
    op.drop_column("audit_logs", "target_id")
    op.alter_column("audit_logs", "target_id_str", new_column_name="target_id", nullable=True)
