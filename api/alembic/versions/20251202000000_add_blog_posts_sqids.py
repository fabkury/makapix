...
def upgrade() -> None:
    ...
    op.add_column(
        "blog_posts",
        sa.Column("new_id", sa.Integer(), nullable=True, server_default=text("nextval('blog_posts_new_id_seq')")),
    )
    ...
    op.execute("SELECT setval('blog_posts_new_id_seq', COALESCE((SELECT MAX(new_id) FROM blog_posts), 0) + 1, false)")
    ...
    op.add_column("blog_post_comments", sa.Column("blog_post_id_new", sa.Integer(), nullable=True))
    op.execute("""
    ...
    op.drop_constraint("blog_post_comments_blog_post_id_fkey", "blog_post_comments", type_="foreignkey")
    op.drop_column("blog_post_comments", "blog_post_id")
    op.alter_column("blog_post_comments", "blog_post_id_new", new_column_name="blog_post_id", nullable=False)
    
    # Blog post reactions table - blog_post_id
    op.add_column("blog_post_reactions", sa.Column("blog_post_id_new", sa.Integer(), nullable=True))
    op.execute("""
    ...
    op.drop_constraint("blog_post_reactions_blog_post_id_fkey", "blog_post_reactions", type_="foreignkey")
    op.drop_column("blog_post_reactions", "blog_post_id")
    op.alter_column("blog_post_reactions", "blog_post_id_new", new_column_name="blog_post_id", nullable=False)
    ...
    op.execute("ALTER TABLE blog_posts ALTER COLUMN id SET DEFAULT nextval('blog_posts_new_id_seq')")
    ...
    op.execute(f"GRANT USAGE, SELECT ON SEQUENCE blog_posts_id_seq TO \"{api_worker_user}\"")
    ...
    op.create_foreign_key("blog_post_comments_blog_post_id_fkey", "blog_post_comments", "blog_posts", ["blog_post_id"], ["id"], ondelete="CASCADE")
    ...
    op.create_foreign_key("blog_post_reactions_blog_post_id_fkey", "blog_post_reactions", "blog_posts", ["blog_post_id"], ["id"], ondelete="CASCADE")
    ...
    op.create_unique_constraint("uq_blog_posts_blog_post_key", "blog_posts", ["blog_post_key"])
    ...
    op.create_unique_constraint("uq_blog_posts_public_sqid", "blog_posts", ["public_sqid"])
