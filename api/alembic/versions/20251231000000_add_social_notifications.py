...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "actor_handle", sa.String(50), nullable=True
        ),  # Denormalized for display
        # Notification details
        sa.Column("emoji", sa.String(20), nullable=True),  # For reaction notifications
        sa.Column(
            "comment_id", UUID(as_uuid=True), nullable=True
        ),  # For comment notifications
        sa.Column(
            "comment_preview", sa.Text(), nullable=True
    ...
    op.create_index("ix_social_notifications_post", "social_notifications", ["post_id"])


def downgrade() -> None:
    op.drop_index("ix_social_notifications_post", table_name="social_notifications")
    op.drop_index(
        "ix_social_notifications_user_unread", table_name="social_notifications"
    )
    op.drop_index(
        "ix_social_notifications_user_created", table_name="social_notifications"
    )
