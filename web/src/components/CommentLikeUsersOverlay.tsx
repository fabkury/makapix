...
interface CommentLikeUsersOverlayProps {
  commentId: string;
  isOpen: boolean;
  onClose: () => void;
}

export default function CommentLikeUsersOverlay({
  commentId,
  isOpen,
  onClose,
}: CommentLikeUsersOverlayProps) {
  const [items, setItems] = useState<CommentLikeUserItem[]>([]);
  ...
  useEffect(() => {
    ...
    setLoading(true);
    fetch(`${API_BASE_URL}/api/post/comments/${commentId}/like-users`)
      .then((res) => {
      ...
      .catch((err) => {
        console.error('Failed to load comment like users:', err);
      })
      .finally(() => {
        setLoading(false);
      });
