...
import { decodeWebPAnimation, DecodingProgress } from "../utils/webpDecoder";

const PISKEL_ORIGIN =
  process.env.NEXT_PUBLIC_PISKEL_ORIGIN || "https://piskel-dev.makapix.club";
...
export default function EditorPage() {
  ...
  const [decodingProgress, setDecodingProgress] =
    useState<DecodingProgress | null>(null);
  ...
  useEffect(() => {
    ...
    if (!token) {
      ...
      router.push(
        `/auth?redirect=${encodeURIComponent("/editor" + window.location.search)}`,
      );
      return;
    }
    ...
    (async () => {
      try {
        ...
        if (isWebP) {
          ...
          try {
            const decoded = await decodeWebPAnimation(
              artworkUrl,
              (progress) => {
                if (!cancelled) {
            ...
            if (decoded.isAnimated && decoded.frames.length > 0) {
              ...
              frameDurationsMsRef.current = decoded.frames.map(
                (f) => f.duration,
              );
        ...
        setError(
          err instanceof Error
            ? err.message
            : "Failed to load artwork for editing",
        );
        setIsLoading(false);
        setDecodingProgress(null);
      }
  ...
  const handleReplace = async (data: any) => {
    ...
    try {
      ...
      const postRes = await authenticatedFetch(
        `${API_BASE_URL}/api/p/${data.originalPostSqid}`,
      );
      if (!postRes.ok) throw new Error("Failed to find original post");
      const post = await postRes.json();

      // Replacing the artwork drops any attached .mkpx layers file
      // server-side (docs/mkpx-upload/ D4) — warn before proceeding.
      if (post.has_mkpx) {
        const proceed = window.confirm(
          "This post has a layers (.mkpx) file attached. Replacing the " +
            "artwork will remove it, since it would no longer match. Continue?",
        );
        if (!proceed) return;
      }
            ...
            <div className="decoding-panel">
              ...
              <div className="decoding-text">
                ...
                {decodingProgress.phase === "decoding" && (
                  <>
                    Loading animation... Frame {decodingProgress.current}/
                    {decodingProgress.total}
                  </>
                )}
                {decodingProgress.phase === "converting" && (
                  <>
                    Converting frames... {decodingProgress.current}/
                    {decodingProgress.total}
                  </>
                )}
              </div>
