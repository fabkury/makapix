...
# Self-heal: the passwords file persists via the bind mount, so actively remove
# the retired shared browser account (2026-08, docs/notification-architecture/).
# Tolerant of absence — mosquitto_passwd -D exits non-zero when the user is gone.
mosquitto_passwd -D "${PASSWD_FILE}" webclient 2>/dev/null || true
