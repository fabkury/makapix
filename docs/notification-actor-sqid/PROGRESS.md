...
- **2026-07-20 (later)** — **LIVE on prod** via PR #242 (merge 9da6039, `make deploy` from `/opt/makapix`): prod REST verified returning `actor_public_sqid` on historical rows; prod web bundle serves the new notifications page. App-repo kickoff status updated to LIVE (afac883). The app team acked the contract and adopted it same-day (0002 in `messages/`; app commit 2fa046a: tappable actor avatar, null → inert; ships in the release after 1.0.16). 0002's only requested reply — the "live on prod" follow-up — delivered via the app-repo kickoff status update (afac883). App consumes notifications via REST polling only (MQTT is their phase C5); the field rides both channels regardless. Remaining: owner click-test on web + app.
